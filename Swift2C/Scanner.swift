...
public class Scanner {
	...
	func CheckLiteral() {
		switch t.val {
			case "class": t.kind = 10
			case "func": t.kind = 14
			case "Int": t.kind = 17
			case "Bool": t.kind = 18
			case "Double": t.kind = 19
			case "String": t.kind = 20
			case "Character": t.kind = 21
			case "Any": t.kind = 22
			case "var": t.kind = 23
			case "let": t.kind = 24
			case "if": t.kind = 26
			case "else": t.kind = 27
			case "while": t.kind = 28
			case "repeat": t.kind = 29
			case "case": t.kind = 30
	...
	func NextToken() -> Token {
		...
		loop: repeat {
			...
			case 1:
				...
				if ch >= "0" && ch <= "9" || ch >= "A" && ch <= "Z" || ch >= "a" && ch <= "z" { AddCh(); state = 1 }
				else { t.kind = 1;  t.val = tval; CheckLiteral(); return t }
			case 2:
				if ch >= "0" && ch <= "9" || ch >= "A" && ch <= "F" || ch >= "a" && ch <= "f" { AddCh(); state = 3 }
				else { state = 0 }
			case 3:
				recEnd = pos; recKind = 2
				if ch == "P" || ch == "p" { AddCh(); state = 4 }
				else if ch >= "0" && ch <= "9" || ch >= "A" && ch <= "F" || ch == "_" || ch >= "a" && ch <= "f" { AddCh(); state = 3 }
				else { t.kind = 2; break loop }
			case 4:
				if ch >= "0" && ch <= "9" { AddCh(); state = 6 }
				else if ch == "+" || ch == "-" { AddCh(); state = 5 }
				else { state = 0 }
			case 5:
				if ch >= "0" && ch <= "9" { AddCh(); state = 6 }
				else { state = 0 }
			case 6:
				recEnd = pos; recKind = 2
				if ch >= "0" && ch <= "9" || ch == "_" { AddCh(); state = 6 }
				else { t.kind = 2; break loop }
			case 7:
				if ch >= "0" && ch <= "9" { AddCh(); state = 8 }
				else { state = 0 }
			case 8:
				recEnd = pos; recKind = 3
				if ch == "E" || ch == "e" { AddCh(); state = 9 }
				else if ch >= "0" && ch <= "9" || ch == "_" { AddCh(); state = 8 }
				else { t.kind = 3; break loop }
			case 9:
				if ch >= "0" && ch <= "9" { AddCh(); state = 11 }
			...
			case 12:
				if ch >= "0" && ch <= "7" { AddCh(); state = 13 }
				else { state = 0 }
			case 13:
				recEnd = pos; recKind = 4
				if ch >= "0" && ch <= "7" || ch == "_" { AddCh(); state = 13 }
				else { t.kind = 4; break loop }
			case 14:
				if ch >= "0" && ch <= "1" { AddCh(); state = 15 }
				else { state = 0 }
			case 15:
				recEnd = pos; recKind = 6
				if ch >= "0" && ch <= "1" || ch == "_" { AddCh(); state = 15 }
				else { t.kind = 6; break loop }
			case 16:
				if ch <= "\t" || ch >= "\u{0B}" && ch <= "\u{0C}" || ch >= "\u{0E}" && ch <= "!" || ch >= "#" && ch <= "[" || ch >= "]" && ch <= "\u{0FFFF}" { AddCh(); state = 16 }
				else if ch == "\"" { AddCh(); state = 18 }
				else if ch == "\\" { AddCh(); state = 17 }
				else { state = 0 }
			case 17:
				if ch >= " " && ch <= "~" { AddCh(); state = 16 }
				else { state = 0 }
			case 18:
				 t.kind = 8; break loop 
			case 19:
				if ch <= "\t" || ch >= "\u{0B}" && ch <= "\u{0C}" || ch >= "\u{0E}" && ch <= "&" || ch >= "(" && ch <= "[" || ch >= "]" && ch <= "\u{0FFFF}" { AddCh(); state = 20 }
				else if ch == "\\" { AddCh(); state = 21 }
				else { state = 0 }
			case 20:
				if ch == "'" { AddCh(); state = 23 }
				else { state = 0 }
			case 21:
				if ch >= " " && ch <= "~" { AddCh(); state = 22 }
				else { state = 0 }
			case 22:
				if ch >= "0" && ch <= "9" || ch >= "A" && ch <= "F" || ch >= "a" && ch <= "f" { AddCh(); state = 22 }
			...
			case 24:
				...
				if ch >= "0" && ch <= "9" || ch == "_" { AddCh(); state = 24 }
				else if ch == "." { AddCh(); state = 7 }
				else { t.kind = 7; break loop }
			case 25:
				recEnd = pos; recKind = 7
				if ch >= "0" && ch <= "9" || ch == "_" { AddCh(); state = 24 }
				else if ch == "x" { AddCh(); state = 26 }
				else if ch == "." { AddCh(); state = 7 }
				else if ch == "o" { AddCh(); state = 12 }
				else if ch == "b" { AddCh(); state = 14 }
				else { t.kind = 7; break loop }
			case 26:
				if ch >= "0" && ch <= "9" || ch >= "A" && ch <= "F" || ch >= "a" && ch <= "f" { AddCh(); state = 27 }
				else { state = 0 }
			case 27:
				recEnd = pos; recKind = 5
				if ch >= "0" && ch <= "9" || ch >= "A" && ch <= "F" || ch == "_" || ch >= "a" && ch <= "f" { AddCh(); state = 27 }
